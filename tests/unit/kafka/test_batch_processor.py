...
async def test_batch_by_size():
    ...
    await processor.add("k1", "v1")
    await processor.add("k2", "v2")
    handler.assert_not_called()

    await processor.add("k3", "v3")
    handler.assert_called_once_with({"k1": "v1", "k2": "v2", "k3": "v3"})
...
async def test_batch_by_timeout():
    ...
    handler.assert_called_once_with({"k1": "v1"})
...
async def test_timer_cancellation_on_size_batch():
    ...
    handler.assert_called_once_with({"k1": "v1", "k2": "v2"})
...
async def test_multiple_batches():
    ...
    await processor.add("k3", "v3")
    await processor.add("k4", "v4")
    handler.assert_called_once_with({"k3": "v3", "k4": "v4"})


@pytest.mark.asyncio
async def test_overlapping_keys_in_batch():
    handler = AsyncMock()
    processor = BatchProcessor(handler, batch_timeout_ms=1000, batch_size=2)

    await processor.add("k1", "v1")
    await processor.add("k1", "v2")  # Same key, should overwrite

    # Size check is on dict length
    assert len(processor._batch) == 1
    handler.assert_not_called()

    await processor.add("k2", "v3")
    handler.assert_called_once_with({"k1": "v2", "k2": "v3"})
